...
from EARS.localization.python_projection.Projector import Projector
from EARS.localization.python_projection.Constraints import Constraints
from EARS.localization.python_projection import Evaluator
from scipy import io
from EARS.localization.python_projection.utils import interpolate
...
def proj_handler(s0, num_iters, alpha=3.4, beta=0.17, disp=False):
    ...
    # Scanner Params([Lustig et al, IEEE TMI 2008])
    dt = 0.00033291878245085695
    dt_inner = 1  # sampling time in ms
    start = time.time()
    ...
    kc = [Constraints(Evaluator.LInf2Norm(), 0.33*alpha * dt, dt_inner, 0),
          Constraints(Evaluator.LInf2Norm(), beta * (dt**2), dt_inner, 1)]

...
def train(args):
    ...
    for epoch in range(start_epoch, args.num_epochs):
        opt_rotor = epoch//(args.cycle)

        if opt_rotor < args.num_rotors and epoch and not epoch % args.cycle:
            if (not args.no_copy):
                model.phase_model.amplitudes.requires_grad = False
                model.phase_model.amplitudes[opt_rotor] = model.phase_model.amplitudes[opt_rotor - 1]
                model.phase_model.amplitudes.requires_grad = True
        ...
        if epoch>=4*args.cycle and not epoch % args.lr_step_size:
            optimizer.param_groups[1]['lr'] *= args.lr_gamma

        if epoch >= 4*args.cycle and not epoch % args.sub_lr_time:
            optimizer.param_groups[0]['lr'] = max(args.sub_lr_stepsize * optimizer.param_groups[0]['lr'], 5e-6)
        ...
        if args.use_orientation:
            train_loss, train_time, train_rec_loss, train_vel_loss, train_acc_loss, avg_train_localization_loss, avg_train_orientation_loss = train_epoch(
                ...
                loss_fn,
                optimizer,
                len(train_loader),opt_rotor=opt_rotor)
        else:
    ...
    if args.use_orientation:
        ...
        if args.test_non_aggregating_trained and args.num_epochs == 1:
            ...
            train_vel_loss = None
            train_acc_loss = None
        test_loss, test_time, test_std, _ = evaluate(args, epoch + 1, model, enum_test, loss_fn,
                                            len(test_loader),
                                            train_loss,
                                                train_rec_loss, train_vel_loss, train_acc_loss, inject_noise_in_sound=False)
        logging.info(
...
def create_arg_parser():
    ...
    parser.add_argument('--sub-lr-time', type=float, default=20,
                        help='learning rate decay timestep of the phase layer')
    parser.add_argument('--sub-lr-stepsize', type=float, default=0.5,
    ...
    parser.add_argument('--use-orientation-aggregate-mlp', action='store_true', default=True, help='Whether to use OrientationAggregateTransformerMLP')
    ...
    parser.add_argument('--position-number', type=int, default=2, help='Choose how many positions to use per coordinate')
    ...
    parser.add_argument('--system-noise-snr-db', type=float, default=0.1, help='Set the std of the white noise injected downstream of the PID controller response')

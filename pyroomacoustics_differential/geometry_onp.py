...
def area(corners):
    ...
    corners = onp.array(corners)
    x = corners[0, :] - onp.roll(corners[0],1)
    y = corners[1, :] + onp.roll(corners[1],1)
    area = -0.5 * (x * y).sum()
    return area
    
def side_onp(p, p0, vector):
    '''
    ...
    p = onp.array(p)
    p0 = onp.array(p0)
    vector = onp.array(vector)
    
    projection = onp.dot(vector, p - p0)
    if (projection > 0):
        return 1
    elif (projection < 0):
        return -1
...
def ccw3p(p1, p2, p3):
    ...
    p1, p2, p3 = onp.array(p1), onp.array(p2), onp.array(p3)
    ...
    #print(f"[debug] computed d {d} eps {consts['eps']} points {p1,p2,p3}")

    if (onp.abs(d) < consts['eps']):
        return 0
    elif (d > 0):
        return 1
    else:
...
def intersection_segment_plane(a1, a2, p, normal):
    ...
    '''
    a1=onp.array(a1).astype(float)
    a2=onp.array(a2).astype(float)
    p=onp.array(p).astype(float)
    normal=onp.array(normal).astype(float)
    if (a1.shape[0] != 3):
...
def is_inside_2D_polygon(p, corners):
    ...
    p0 = onp.array([onp.amin(corners[0])-1, p[1]])
...
def intersection_2D_segments_onp(a1, a2, b1, b2) :
    '''
    ...
    p = onp.array((num / denom.astype(float))*db + b1)
...
def intersection_segment_polygon_surface_onp(a1, a2, corners_2d, normal, plane_point, plane_basis):
    '''
    ...
    a1 = onp.array(a1)
    a2 = onp.array(a2)
    corners_2d = onp.array(corners_2d)
    normal = onp.array(normal)
    if (a1.shape[0] != 3):
    ...
    localp = onp.dot(plane_basis.T, p - plane_point)
...
def circular_2D_array(center, M, phi0, radius):
    ...
    poses = onp.array(center)[:, onp.newaxis] + radius * \
        onp.vstack((onp.cos(phi + phi0), onp.sin(phi + phi0)))
    return poses, phi

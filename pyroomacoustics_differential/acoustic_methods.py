...
from .room import wall_intersection, wall_side, walls_intersects
import itertools
from .consts import consts
from .geometry import ccw3p
...
def is_obstructed(room, source, p, imageId = 0):
    ...
    for wallId in room['obstructing_walls']:
        ...
        if(wallId != genWallId):
            ...
            intersectionPoint, borderOfSegment, borderOfWall = wall_intersection(room['walls'][wallId],source['images'][:, imageId], p)
...
def first_order_images(room, source_position):
    ...
    # This second alternative more closely reflects the numpy version!
    ...
    # This second alternative more closely reflects the numpy version!
    #wall_indices = jnp.arange(len(room['walls']))[ip>0]
...
def image_source_model(room, sources, mics_array, max_order=1):
    ...
    for i, source in enumerate(sources):
        print(f"Computing images for source {i+1}/{len(sources)}", flush=True)
        # pure python
        if max_order > 0:

            # generate first order images 
            # (this contains a difference compared to the numpy version, see in particular 
            # when we use jnp.where)
...
def compute_rir_threaded(sources, mics_array, visibility, room_fs, room_t0, max_order):
    ...
    for m, mic in enumerate(mics_array['R'].T):
        ...
        with ThreadPoolExecutor(max_workers=20) as pool:
            for h_s in pool.map(lambda p: get_rir_jax(p[0], mic, p[1][m], p[2], p[3], p[4]), args_list):
                h.append(h_s[0])
                for i in range(max_order+1):
                    h_by_orders[i].append(h_s[1][i])
...
def compute_rir_jax(sources, mics_array, visibility, room_fs, room_t0):
    ...
    rir_arr = jnp.reshape(jnp.asarray([jnp.pad(i, (0, int(max_len_rir)-len(i))) for i in hs]), (mics_array['M'],len(sources),max_len_rir))
    return rir_arr
...
@jit
def batch_convolve1d_circular_boundary(x, y):
    # batch_convolve1d with circular boundary conditions
    # x: 1d array
    # y: batch of N 1d arrays
    # out: Nx(np.convolve(x,y[i]).shape[0])
    # based on convolve1d implemented as in https://github.com/google/jax/issues/1561
    y_flipped = jnp.flip(jnp.reshape(y, (y.shape[0], 1, y.shape[1])),2)
    x_padded = jnp.pad(x, pad_width=[(y_flipped.shape[2] - 1, y_flipped.shape[2] - 1)], mode="wrap")
    x_reshaped = jnp.reshape(x_padded,(1,1,len(x_padded)))
    ...
    res = lax.conv_general_dilated(
        ...
        y_flipped,
        window_strides=(1,),
        padding=[(0,0)])  # equivalent of padding='full' in NumPy
    
    return res

@jit
def batch_convolve1d_no_padding(x, y):
    # x: 1d array
    # y: batch of N 1d arrays
    # out: Nx(np.convolve(x,y[i]).shape[0])
    # based on convolve1d implemented as in https://github.com/google/jax/issues/1561
...
def modified_simulate(rir_at_loc, signals, delay_sources, num_sources, num_mics, max_rec_len):
    ...
    premix_signals = jnp.zeros((num_sources, num_mics, max_rec_len))
    ...
    for s in range(num_sources):
        ...
        conv_res = batch_convolve1d_no_padding(sig, h)[0]

        #premix_signals = index_add(premix_signals, index[s, :, delay_sources:delay_sources + len(sig) + h.shape[1] - 1], conv_res)
        # premix_signals = premix_signals.at[s, :, delay_sources:delay_sources + len(sig) + h.shape[1] - 1].add(conv_res)
        premix_signals = premix_signals.at[s, :, delay_sources:delay_sources + len(sig) - h.shape[1] + 1].add(conv_res)
...
def simulate(rir_at_loc, sources, mics_array, room_fs):
    ...
    for m in jnp.arange(num_mics):
        for s in jnp.arange(num_sources):
            ...
            conv_res = signal.convolve(h, sig) # prior the implementation of jax.scipy.signal.convolve was using jnp.convolve

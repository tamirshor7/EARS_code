...
def area(corners):
    ...
    x = corners[0, :] - jnp.roll(corners[0],1)
    y = corners[1, :] + jnp.roll(corners[1],1)
    area = -0.5 * (x * y).sum()
    return area
...
def side(p, p0, vector):
    ...
    projection = jnp.dot(vector, p - p0)
    if (projection > 0):
        return 1
    elif (projection < 0):
        return -1
...
def ccw3p(p1, p2, p3):
    ...
    p1, p2, p3 = jnp.array(p1), jnp.array(p2), jnp.array(p3)
...
def intersection_2D_segments(a1, a2, b1, b2) :
    ...
    da = a2-a1
    db = b2-b1
    dap = jnp.empty_like(da)
    # dap = index_update(dap, 0, -da[1])
    # dap = index_update(dap, 1, da[0])
    dap = dap.at[0:2].set([-da[1], da[0]])

    denom = jnp.dot(dap, db)
    ...
    p = jnp.array((num / denom.astype(float))*db + b1)
...
def intersection_segment_plane(a1, a2, p, normal):
    ...
    a1=jnp.array(a1).astype(float)
    a2=jnp.array(a2).astype(float)
    p=jnp.array(p).astype(float)
    normal=jnp.array(normal).astype(float)
    if (a1.shape[0] != 3):
    ...
    denom = jnp.dot(normal, u)

    if(abs(denom) < consts['eps']):
        ...
        num = -jnp.dot(normal, w)
        s = num/denom

        if s < -consts['eps'] or s > 1 + consts['eps']:
            # in this case, both points are on the same side of the plane
            # hence, no intersections
            return None, False
        else:
            if abs(s) < consts['eps'] or abs(s - 1) < consts['eps']:
                limitCase = True
            else:
                limitCase = False
...
def is_inside_2D_polygon(p, corners):
    ...
    p0 = jnp.array([jnp.amin(corners[0])-1, p[1]])
...
def intersection_segment_polygon_surface(a1, a2, corners_2d, normal, plane_point, plane_basis):
    ...
    a1 = jnp.array(a1)
    a2 = jnp.array(a2)
    corners_2d = jnp.array(corners_2d)
    ...
    localp = jnp.dot(plane_basis.T, p - plane_point)
...
def circular_2D_array_jax(center, M, phi0, radius):
    '''
    ...
    poses = jnp.array(center)[:, jnp.newaxis] + radius * \
        jnp.vstack((jnp.cos(phi + phi0), jnp.sin(phi + phi0)))
    return poses, phi

circular_2D_array_jax_jitted = jit(circular_2D_array_jax, static_argnums=(1))


def ellipse_2D_array(center, M, a, b):
    # x = center[0] + a cos(t) ; y = center[1] + b sin(t)
    #phi = onp.linspace(0, 2*onp.pi, M)
    phi = onp.arange(M) * 2. * onp.pi / M
    poses = onp.array(center)[:, onp.newaxis] + \
        onp.vstack((a*onp.cos(phi), b*onp.sin(phi)))
    return poses, phi
...
def circular_2D_array(center, M, phi0, radius):
    ...
    ndarray (2, M)
        The array of points
    '''
    # in our case M=0, hence phi is an empty array
    phi = onp.arange(M, dtype=onp.float64) * 2. * onp.pi / M
    poses = onp.array(center)[:, onp.newaxis] + radius * \
        onp.vstack((onp.cos(phi + phi0, dtype=onp.float64), onp.sin(phi + phi0, dtype=onp.float64)))
    return poses, phi

def linear_array_based_dist_from_org(center, distances, phi0):
    poses = onp.array(center)[:, onp.newaxis] + distances *\
        onp.vstack((onp.cos(phi0), onp.sin(phi0)))
    if onp.size(phi0) == 1:
        phi = onp.tile(phi0, distances.shape[0])
        return poses, phi
    else:
        # in generating simulated recording size = 640
        return poses, phi0

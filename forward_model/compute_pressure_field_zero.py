...
def process_directory(rir_directory_path, saving_directory_path, input_sound, rotor_position):
    ...
    print(f'final shape: {grid_numpy_raw.shape}')

    onp.save(os.path.join(saving_directory_path, 'pressure_field_raw.npy'), grid_numpy_raw)
    print(f"Pressure field saved in {os.path.join(saving_directory_path, 'pressure_field_raw.npy')}")
    

    process_pressure_field(grid_numpy_raw, saving_directory_path)
...
saving_directory_path_free_space = os.path.join(saving_directory_path_prefix, 'free_space_zero')
if not os.path.exists(saving_directory_path_free_space):
    os.makedirs(saving_directory_path_free_space)
saving_directory_path_indoor = os.path.join(saving_directory_path_prefix, 'indoor_zero')
...
phase_modulation = onp.zeros((4,1025))
phase_modulation_torch = torch.from_numpy(phase_modulation)

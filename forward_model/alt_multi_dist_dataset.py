...
from numpy import savez_compressed, float32, squeeze
from tqdm.auto import tqdm

'''
In this file we try to create a dataset that contains the data from several distances
in a slight different way from multi_dist_dataset.py (in particular, we set the phases to be equal to 1)
'''
...
output_dir_path = os.path.join(cur_dir,'..','..','data','alt_multi_distance_dataset/')
...
# num_phases is the number of phases that we want to use in the forward model
# It represents the number of virtual microphones that we want to use for each radius distance (which
# represents the number of real microphones that we have)
num_phases = 1
for i,(channel, recording_mode, vel_rate) in tqdm(enumerate(product(channel_values, recording_mode_values, vel_rate_values))):
    ...
    # Convert encoder_readings to Tensor AND pick just the first entry (all of the other
    # entries are the same as they are repeated for each microphone, virtual or real)
    encoder_readings = from_numpy(squeeze(encoder_readings.astype(float32)[0,:]))

    # We take the recordings of every distance by skipping num_phases entries (which represents
    # the virtual microphones)
    real_recordings = real_recordings.astype(float32)

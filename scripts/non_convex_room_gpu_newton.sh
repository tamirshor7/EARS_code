...
python EARS/forward_model/forward_indoor_wrapper.py -max_order 1 \
    ...
    -flip_rotation_direction 1 0 0 1 -gpu 0 \
    -number_of_angles 64 -compute_sound_only \
    -chunk "$chunk" -total_gpus_available "$total_number_of_available_gpus"  -fix_corrupted \
    -compress_data -user "$user" -use_newton_cluster \
